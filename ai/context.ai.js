...
import { userContextBuilder } from "./builders/context.builder.js";
...
export const getWeeklyContextThemes = async (contextObj) => {
  try {
    ...
    const contextString = userContextBuilder(contextObj);
...
// update monthly themes in user context

export const getMonthlyContextThemes = async (contextObj) => {
  try {
    const pretext =
      "As a CBT therapist, analyze the user's monthly data including mood entries, journal entries, and chat history. " +
      "Identify patterns, recurring topics, and areas of focus across all three categories. " +
      "Your response MUST be in valid JSON format following this exact structure:\n\n" +
      "```json\n" +
      "{\n" +
    ...
    console.log(`Error updating monthly context themes: ${error.message}`);
  }
};
